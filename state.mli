...
(** [get_dealer_hand_value players] gets the dealer's hand value from a list 
    of all the players in state.  *)
...
(** [hit player g] is [r] after a  player [player] chooses to hit. 
    ...
    drawn, then [r] is [Legal g'], i.e. the new state of the game with the 
    player's hand and hand value updated, and the drawn card removed from the 
    deck.  
...
(** [in_stayed player g] returns whether a player [player] is in ths stayed 
    list of the game *)
...
(** [game_end_status d p] returns whether the player won, lost, tied, or got 
    a blackjack at the end of the game. *)
...
(** [get_player g] is all the non-dealer, non main player [players] in the 
    game state g. It is all the AI units. *)
val get_other_players: t -> Player.t list

(** [split player g] the result [r] after player [player] splits in game 
    state [g]*)
val split: Player.t -> t -> result 

(** [dealer_top_card g] is the top card [card] of the dealer in game 
    state [g]. *)
